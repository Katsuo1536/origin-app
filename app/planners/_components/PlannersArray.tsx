    ...
    <>
      ...
      <div className="grid grid-cols-2 justify-items-center gap-10 w-fit mx-auto">
        ...
        {values?.map(elem => (
          ...
          <Link key={elem.id} href={`/planners/${elem.id}`}>
            <section className="rounded-2xl border-2 border-orange-400 h-45 w-55 gap-3 py-1">
              <span className="flex justify-start items-start text-gray-700 text-sm font-midium rounded-2xl px-2 py-1">
                {time(elem.date)}
              </span>
